...
<div>
    <label for="<?= $name ?>" class="block text-sm text-gray-400 mb-1.5"><?= e($label) ?></label>
    <div class="<?= $password ? 'relative' : '' ?>">
        <input type="<?= $type ?>"
               ...
               value="<?= e($value) ?>"
               <?= $required ? 'required' : '' ?>
               class="w-full bg-dark-900 border border-dark-600 text-white rounded-lg px-4 py-2.5 <?= $password ? 'pr-11' : '' ?> focus:outline-none focus:border-accent-500 focus:ring-1 focus:ring-accent-500 transition-colors duration-200 placeholder-gray-500"
